...
var debugr = document.getElementById("debug-top");
//debugr.innerHTML = 'top: ' + flip.css('top') + '<br>left: ' + flip.css('left');
//debugr.innerHTML = 'top: ' + options.flipbook.css('top') + '<br>left: ' + options.flipbook.css('left');
//debugr.innerHTML = 'top: ' + move.y + '<br>left: ' + move.x;
...
/*
  ...
  do{
	...
	tag += obj.tagName + ": " + obj.offsetLeft + "\n";
 ...
 */
	...
	// hack yves
    //var flipRealPos = {left: flip[0].offsetParent.offsetLeft, top: flip[0].offsetParent.offsetTop};


      
    move = point2D(
      -flip.width()/2 - flipRealPos.left + tmp.width()/2 + flipDesPos.x + thisOffset.left,
      //-flip.width()/2 - flip.position().left/2 + tmp.width()/2 + flipDesPos.x,
      //flip.position().left/ - flipDesPos.x,
      //-1620,
    ...
    if (duration===0) {
      ...
      flip.bind(transitionEnd, function() {
        ...
        transitionEndCallback();
		  ...
		  //alert("before translate 4");

      //flip.transform(scale(scaling, true));
